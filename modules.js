...
exports.getUserInp = function(username){
    return db.query(
        'SELECT first, last, id FROM users WHERE username =($1)',
        [username]
    ).then((results) => {
...
exports.getCity= function(city) {
    ...
    LEFT JOIN user_profiles
    ON users.id = user_profiles.user_id
    WHERE LOWER(user_profiles.city) = LOWER($1)`, [city])
            .then(function(results) {
                resolve(results.rows);
            }).catch(function(err) {
                reject(err);
...
exports.getDetails = function(user) {
    return new Promise(function(resolve, reject) {
        ...
        LEFT JOIN user_profiles
        ON users.id = user_profiles.user_id
        WHERE user_profiles.user_id = ($1)`, [user])
            .then(function(results) {
